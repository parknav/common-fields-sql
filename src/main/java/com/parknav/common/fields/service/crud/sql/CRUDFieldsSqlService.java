...
import com.steatoda.nar.service.crud.NarCRUDException;
...
public abstract class CRUDFieldsSqlService<I, C extends NarEntity<I, C, F>, F extends Enum<F> & NarField, S> implements NarCRUDService<I, C, F, S> {
	...
	public C get(I id, NarGraph<F> graph) {
		...
		try (PreparedStatement statement = new SelectStatement(tableName)
			...
			throw new NarCRUDException("Unable to find entity with id " + id, e);
		}
		...
		try (PreparedStatement statement = buildCreateStatement(entity, graph).build(getConnection())) {
			if (statement.execute())
				try (ResultSet resultSet = statement.getResultSet()) {
					if (!resultSet.next())
						throw new NarCRUDException("Unable to insert new entity into db!");
			...
			throw new NarCRUDException("Unable to create entity", e);
		}
	...
	protected InsertStatement buildCreateStatement(C entity, NarGraph<F> graph) {
	...
	public void modify(C entity, C patch, NarGraph<F> graph) {
		...
		try (PreparedStatement statement = updateStatement.build(getConnection())) {
			if (statement.execute())
				try (ResultSet resultSet = statement.getResultSet()) {
					if (!resultSet.next())
						throw new NarCRUDException("Unable to update entity " + entity);
					readResultSet(entity, resultSet, graph);
				}
		} catch (SQLException e) {
			throw new NarCRUDException("Unable to modify entity " + entity, e);
		}
	...
	/**
	 * Constructs statement used in {@link #modify(NarEntity, NarEntity, NarGraph)}.
	 *
	 * @param entity entity to modify
	 * @param patch patch describing attributes to modify
	 * @param graph return graph
	 ...
	 */
	protected UpdateStatement buildModifyStatement(C entity, C patch, NarGraph<F> graph) {
	...
	public void delete(C entity) {
		...
		try (PreparedStatement statement = new DeleteStatement(tableName)
			...
			throw new NarCRUDException("Unable to delete entity " + entity, e);
		}
	...
	public Stream<C> queryAllFieldValues(S selector, Set<F> fields) {
		...
		try {
			...
			return StreamSupport.stream(new FieldsStatementSpliterator<>(statement, this::resolveResultSet, NarGraph.of(fields)), false)
				.peek(entity -> entity.setId(null));	// #getDQLColumns probably added id and #readResultSet read it, but it's meaningless
		} catch (SQLException e) {
			throw new NarCRUDException("Unable to list all values for fields " + fields, e);
		}
	...
	public int count(S selector) {
		...
		try (PreparedStatement statement = new SelectStatement(tableName)
			...
			try (ResultSet resultSet = statement.executeQuery()) {
				if (!resultSet.next())
					throw new NarCRUDException("Query did not return any rows?!!");
				return resultSet.getInt(1);
			}
		} catch (SQLException e) {
			throw new NarCRUDException("Unable to count entities", e);
		}
	...
	protected SelectStatement buildQueryStatement(S selector, NarGraph<F> graph) {
		return buildQueryStatement(selector, graph, SelectStatement::new);
	}

	protected SelectStatement buildQueryStatement(S selector, NarGraph<F> graph, Function<String, SelectStatement> selectStatementBuilder) {
		return selectStatementBuilder.apply(tableName)
			.addTableDQLColumns(getDQLColumns(graph), this::decorateDQLColumn)
			.addWhereTerms(getSelectorWhereTerm(selector));
	}

	protected Stream<C> doQuery(SelectStatement select, NarGraph<F> graph) {
		...
		try {
			...
			throw new NarCRUDException("Unable to list entities", e);
		}
	...
	protected abstract void readResultSet(C entity, ResultSet resultSet, NarGraph<F> graph) throws SQLException;
	...
	protected C resolveResultSet(ResultSet resultSet, NarGraph<F> graph) throws SQLException {
		C entity = instance();
		readResultSet(entity, resultSet, graph);
		return entity;
	}
